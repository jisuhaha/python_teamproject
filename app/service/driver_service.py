...
def driver_assign_service():
    ...
    selSQL = "select groupname from board where oid = {0}".format(boardOID)
    conn = DB('dict')
    result = conn.select_one(selSQL, None)
    logs.logger.info(f'{name}기사님의 {result.get('groupname')}운송이 지정되었습니다.')
